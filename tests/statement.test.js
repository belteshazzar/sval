...
describe('testing src/index.ts', () => {
  ...
  it('should for-await-of statement run normally', async () => {  
    const interpreter = new Sval()
    function getItem(n) {
      return new Promise(resolve => setTimeout(resolve, 5, n))
    }
    await new Promise((resolve) => {
      interpreter.import({ getItem, expect, done: resolve })
      interpreter.run(`
        b()
        async function* a() {
          for (const i of [1, 2, 3]) {
  ...
  it('should for-await-of with manual iterator run normally', async () => {  
    const interpreter = new Sval()
    await new Promise((resolve) => {
      interpreter.import({ expect, done: resolve })
      interpreter.run(`
        c()
  ...
  it('should support for-await-of with sync iterables', async () => {  
    const interpreter = new Sval()
    await new Promise((resolve) => {
      interpreter.import({ expect, done: resolve })
      interpreter.run(`
  ...
  it('should support for-await-of with sync iterator', async () => {  
    const interpreter = new Sval()
    await new Promise((resolve) => {
      interpreter.import({ expect, done: resolve })
      interpreter.run(`
  ...
  it('should support for-await-of with async iterator', async () => {  
    const interpreter = new Sval()
    await new Promise((resolve) => {
      interpreter.import({ expect, done: resolve })
      interpreter.run(`
  ...
  it('should support for-await-of with async generator', async () => {  
    const interpreter = new Sval()
    await new Promise((resolve) => {
      interpreter.import({ expect, done: resolve })
      interpreter.run(`
